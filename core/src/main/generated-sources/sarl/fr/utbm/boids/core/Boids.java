...
import fr.utbm.boids.core.ResultatDeplacement;
...
public class Boids extends Agent {
  ...
  private void $behaviorUnit$Initialize$0(final Initialize occurrence) {
    ...
    _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.info("Boids agent was started.");
  ...
  private void $behaviorUnit$DemandeDeplacement$1(final DemandeDeplacement occurrence) {
    Collection<Boids> otherBoids = occurrence.otherBoids;
    boolean _notEquals = (!Objects.equal(otherBoids, null));
    if (_notEquals) {
      Vecteur force = null;
      Vecteur influence = null;
      Vecteur _vecteur = new Vecteur(0, 0);
      force = _vecteur;
      force = this.separation(otherBoids);
      this.cohesion();
      this.alignement();
      this.repulsion();
    }
    DefaultContextInteractions _$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS$CALLER = this.$castSkill(DefaultContextInteractions.class, (this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS == null || this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS.get() == null) ? (this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS = this.$getSkill(DefaultContextInteractions.class)) : this.$CAPACITY_USE$IO_SARL_CORE_DEFAULTCONTEXTINTERACTIONS);
    ResultatDeplacement _resultatDeplacement = new ResultatDeplacement(this.position);
